...
function updateControlDisplay(item, alert_status) {

    if (alert_status === 'pausing'){
      pauseBtn.textContent = 'Resume';
      pauseBtn.style.backgroundColor = '#38e60d';
    } else {
      pauseBtn.textContent = 'Pause';
...
// Update the countdown labels and each camera item with the latest data
function update_Display () {
    ...
    console.warn(`ALERT STATUS  ${alert_status}`);
...
(async function init() {
  ...
  //Get a list of all camera rows
    ...
    update_Display();
    setInterval(update_Display, 5000);
})();
...
async function getCountdownSettings() {
  try {
    ...
    if (!res.ok) return [];
    const data = await res.json();
    console.warn('COUNTDOWN DATA');
    console.warn(data);
    return data || [];
  } catch {
    return { countdown_action: null, countdown_time: null, countdown_control: null,alert_status: null };
  }
}
